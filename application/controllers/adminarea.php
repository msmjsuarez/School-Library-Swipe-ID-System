...
class Adminarea extends CI_Controller {
	...
	public function enroll_patron()
	{
		...
		$this->form_validation->set_rules('id_number', 'ID Number', 
											'trim|required|min_length[7]|max_length[15]|is_unique[patron.id_number]');
	...
	{
		$this->load->library('form_validation');
		$this->form_validation->set_rules('id_number', 'ID Number', 'trim|required|min_length[7]|max_length[15]');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->load->model('adminarea_model');
				...
				echo '<script>alert("New Patron Grade Level/Section/School Year has been added. Thank you!")</script>';
					...
					echo '<script>alert("Please check entry!")</script>';
					$this->patron_grade_section();
				}
		}
	...
	public function reports_purpose() 
	{
		$this->load->model('adminarea_model');
		$data['query'] = $this->adminarea_model->admin_details();
		
		$data['content'] = 'reports_purpose';
		$this->load->view('includes/template_admin', $data);
	}
	...
	public function edit_patron() 
	{
		$this->load->model('adminarea_model');
		$data['query'] = $this->adminarea_model->admin_details();

		$data['content'] = 'edit_patron';
		$this->load->view('includes/template_admin', $data);
	}
	
	public function edit_patron_pic() 
	{
		$this->load->model('adminarea_model');
		$data['query'] = $this->adminarea_model->admin_details();
		$data['query1'] = $this->adminarea_model->patron_image();
		
		$data['content'] = 'edit_patron_pic';
		$this->load->view('includes/template_admin', $data);
	}
	
	public function edit_patron_pic1()
	{
				  $this->load->model('adminarea_model');
				  $file_element_name = 'picture';
				  ...
				  if (!$this->upload->do_upload($file_element_name))
				  {
					 echo 'Please upload your certificate.';
					 $this->edit_patron_pic();
				  }
				  else
				  {
					 $data = $this->upload->data();
					 $file_id = $this->adminarea_model->patron_image_update($data['file_name'], $_POST['patron_id']);
					 if($file_id) 
					 {
						 echo 'Adding trade certificate successful.';
						 $this->edit_patron_pic();
					 }
					 else
					 {
	...
	} // end of edit_patron_pic1
	
	public function patron_delete()
	{
		$this->load->model('adminarea_model');
		$data['query'] = $this->adminarea_model->patron_delete_submit();
	...
	public function edit_section() 
	{
		...
		$data['content'] = 'edit_section';
		$this->load->view('includes/template_admin', $data);
	}
	
	public function search_school_year() 
	{
		$this->load->model('adminarea_model');
		$data['query'] = $this->adminarea_model->admin_details();
		
		$data['content'] = 'search_school_year';
		$this->load->view('includes/template_admin', $data);
	}
	...
	public function edit_school_year() 
	{
		...
		$data['content'] = 'edit_school_year';
		$this->load->view('includes/template_admin', $data);
	}
	
	public function search_sys_user() 
	{
		$this->load->model('adminarea_model');
		$data['query'] = $this->adminarea_model->admin_details();
		
		$data['content'] = 'search_sys_user';
		$this->load->view('includes/template_admin', $data);
	}
	...
	{
		$this->load->model('adminarea_model');
		$data['query'] = $this->adminarea_model->delete_sys_user_submit();
		
		$this->search_sys_user();
